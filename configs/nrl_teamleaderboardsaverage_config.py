"""
NRL Team Leaderboards Average API — Flattening Config
Source  : api/NRL/teamLeaderBoardsAverage/{competitionId}/{seasonId}/{roundNumber}
Bronze  : sandbox.swathi.teamleaderboardsaverage
Mode    : Standard (l0payload is NULL)
Element : l1element = 'stats'

ingested_from pattern : teamLeaderboardsAverage_{competitionId}_{seasonId}_{roundNumber}
e.g. teamLeaderboardsAverage_111_2026_2

Payload structure (array — IS the leaderboardStat array directly):
  [{
    "ID":       "0",            ← statId
    "statName": "Sin Bin",
    "leaderboardEntry": [       ← array of team average ranking rows
      {
  ...
  Difference from teamLeaderboards: values are per-game averages (e.g. "1.00")
  rather than cumulative totals.

Output tables (target_prefix = sandbox.swathi.nrl_team_leaderboards_average):
  nrl_team_leaderboards_average                   ← Level 0 root
  nrl_team_leaderboards_average_leaderboard_stat  ← Level 1
  nrl_team_leaderboards_average_leaderboard_entry ← Level 2
...
def _write_records(records, table_name, spark, write_mode="overwrite"):
    ...
    if not records:
        print(f"[teamleaderboardsaverage] No records for {table_name} — skipping")
        return
    ...
    print(f"[teamleaderboardsaverage] {len(rows)} rows → {table_name}")


def _create_team_leaderboards_average_tables(config, spark):
    """
    Creates three doc-aligned tables from the teamLeaderboardsAverage bronze data.

    nrl_team_leaderboards_average                   — one row per round ingestion
    nrl_team_leaderboards_average_leaderboard_stat  — one row per stat per round
    nrl_team_leaderboards_average_leaderboard_entry — one row per team per stat per round
    """
    ...
    for row in rows:
        # Parse context: team_leaderboards_average_{competitionId}_{seasonId}_{roundNumber}
        ...
        ctx = {
            ...
            "source_table_name":   "team_leaderboards_average",
        ...
        if root_key not in seen_root:
            ...
            root_records.append({
                **ctx,
                ...
                "target_table_name": "nrl_team_leaderboards_average",
            })
        ...
        for stat in payload:
            ...
            stat_records.append({
                ...
                "stat_name": stat_name,
                **ctx,
                "target_table_name": "nrl_team_leaderboards_average_leaderboard_stat",
            })
            ...
            for entry in leaderboard_entry:
                ...
                entry_records.append({
                    ...
                    **ctx,
                    "target_table_name": "nrl_team_leaderboards_average_leaderboard_entry",
                })
...
TEAMLEADERBOARDSAVERAGE_CONFIG = {
    "bronze_table":  "nrl_datalakehouse_prd.bronze_stats.team_leaderboards_average",
    "target_prefix": "nrl_datalakehouse_prd.bronze_stats_flatten.nrl_team_leaderboards_average",
    ...
    "source_filter_template": "ingested_from LIKE 'team_leaderboards_average_{competition_id}_{season}%'",
    ...
    # Creates: nrl_team_leaderboards_average,
    #          nrl_team_leaderboards_average_leaderboard_stat,
    #          nrl_team_leaderboards_average_leaderboard_entry
    "post_run_fn": _create_team_leaderboards_average_tables,
}
